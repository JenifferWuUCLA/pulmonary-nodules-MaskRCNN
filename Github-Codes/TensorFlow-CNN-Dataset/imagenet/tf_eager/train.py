            ...
            with tf.contrib.summary.record_summaries_every_n_global_steps(self.cfg.DISPLAY_STEP):
                ...
                for e in range(self.epoch.numpy(), self.cfg.EPOCHS):
                    ...
                    for (batch_i, (images, labels)) in enumerate(tfe.Iterator(self.trainingset.dataset)):
                        ...
                        self.optimizer.minimize(lambda: self.loss('train', images, labels),
                        ...
                        if (step % self.cfg.DISPLAY_STEP) == 0:
                            ...
                            print (
                            'Epoch: {:03d} Step/Batch: {:09d} Step mean time: {:04d}ms \nLoss: {:.7f} Training accuracy: {:.4f}'.format(
                                e, step, int(step_time / step), l, a))
                        ...
                        if (step % self.cfg.VALIDATION_STEP) == 0:
                            ...
                            print ('Elapsed time: {} --- Loss: {:.7f} Validation accuracy: {:.4f}'.format(
                                ut.format_time(int_time), l, a))

                        if (step % self.cfg.SAVE_STEP) == 0:
                            tfe.Saver(self.all_variables).save(os.path.join(self.cfg.CKPT_PATH, 'net.ckpt'),
                                                               global_step=self.global_step)
