...
for epoch in range(n_epoch):  # 1个epoch等于使用训练集中的全部样本训练一次
    ...
    data_tr_batch, label_tr_batch = tf.train.shuffle_batch([data_train, label_train], batch_size=batch_size,
    ...
    data_te_batch, label_te_batch = tf.train.shuffle_batch([data_test, label_test], batch_size=batch_size,
                                                           capacity=2000, min_after_dequeue=1000)
    threads = tf.train.start_queue_runners(sess=sess)
    ...
    '''####################训练方法：2#########################'''
    # training
    _, err, ac = sess.run([train_op, loss, accuracy],
                          feed_dict={x: data_tr_batch, y_: label_tr_batch})  # sess.run返回的是在里面填的东西，一一对应
