...
with tf.name_scope('conv1') as scope:  # 将scope内生成的Variable自动命名为conv1/xxx，便于区分不同卷积层之间的组件
    ...
    conv = tf.nn.conv2d(x_image, kernel, [1, 2, 2, 1],
                        padding='SAME')  # 对输入的images完成卷积操作，28x28图片，用5x5的卷积核，每两步一卷积，模式为Same，最后输出的是14x14的图片，计算方法参见网页收藏
...
with tf.name_scope('conv2') as scope:
    kernel = tf.Variable(tf.truncated_normal([3, 3, 8, 32], dtype=tf.float32, stddev=1e-1),
